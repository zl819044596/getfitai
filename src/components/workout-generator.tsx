...
export function WorkoutGenerator() {
  ...
  function handleToggleFavorite() {
    ...
    if (isFavorited) {
      ...
      addFavorite(plan.title)
      setIsFavorited(true)
      trackPlanSaved(plan.title, "favorite")
    }
  }
  ...
  function handleSharePlan() {
    ...
    const shareText = [
      `🏋️ ${plan.title}`,
      `⏱️ ${plan.duration} • 🔥 ${plan.calories} • ${plan.difficulty}`,
      ``,
      `Exercises:`,
      ...plan.exercises.map((ex, i) => `${i + 1}. ${ex.name} — ${ex.sets}x${ex.reps} @ ${ex.weight}`),
      ``,
      `Generated by GetFitAI — Free AI Workout Plans`,
      `https://www.getfitai.io/tools/workout-generator`,
    ].join('\n')
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="shrink-0 p-2 bg-white/20 rounded-lg hover:bg-white/30 transition-colors relative"
                          title={shareCopied ? "Copied!" : "Share plan"}
                        >
                          {shareCopied ? (
                        ...
                        >
                          <Star className={`w-5 h-5 ${isFavorited ? "fill-yellow-400 text-yellow-400" : "text-white"}`} />
                        </button>
                        {/* Reset button */}
                        <button
                          onClick={handleReset}
